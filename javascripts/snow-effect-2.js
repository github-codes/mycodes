...
var embHTML="";
for(i=1;i<200;i++){embHTML+='<i class="embedim-snow"></i>';
...
{background:"+embRandColor2()+";opacity:"+(embRand2(1,1e4)*1e-4).toFixed(2)+";
transform:translate("+rndX.toFixed(2)+"vw,-10px) scale("+rndS+");
animation:fall-"+i+" "+embRand2(10,30)+"s -"+embRand2(0,30)+"s linear infinite}
@keyframes fall-"+i+"{"+rndT+"%{transform:translate("+(rndX+rndO).toFixed(2)+"vw,"+rndT+"vh) 
scale("+rndS+")}to{transform:translate("+(rndX+rndO/2).toFixed(2)+"vw, 105vh) 
scale("+rndS+")}}"}embedimSnow=document.createElement("div");
